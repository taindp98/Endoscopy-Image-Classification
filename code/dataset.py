    ...
    if not is_full_sup:
        if config.TRAIN.IS_SSL:
            if config.DATA.MOCKUP_SSL:
                ...
                num_unl_samples_per_batch = len(df_unlabeled)//(config.DATA.BATCH_SIZE*config.config.DATA.MU)
                df_unlabeled = df_unlabeled.sample(num_unl_samples_per_batch*config.DATA.BATCH_SIZE*config.config.DATA.MU, random_state=1)
            ...
            ## else for real unlabeled data
                ...
                num_unl_samples_per_batch = len(df_unanno)//(config.DATA.BATCH_SIZE*config.config.DATA.MU)
                df_unanno = df_unanno.sample(num_unl_samples_per_batch*config.DATA.BATCH_SIZE*config.config.DATA.MU, random_state=1)
